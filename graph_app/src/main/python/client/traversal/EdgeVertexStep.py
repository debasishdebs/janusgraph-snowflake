...
class EdgeVertexStep(Traversal.Traversal):
    ...
    def with_(self, prop_name, prop_val: Predicates):
        ...
        self.byte_rep[-1].extend(byte_arr) if self.anonymous else self.byte_rep[-1].append(byte_arr)
    ...
    def withLabel(self, label):
        ...
        print("withLabel")
        print(self.byte_rep[-1][-1])
        is_labelized = any([True if len(x) > 0 and "T.label" == x[0] else False for x in self.byte_rep[-1]])
        print(is_labelized)
        print([True if len(x) > 0 and "T.label" == x[0] else False for x in self.byte_rep[-1]])
        print([x[0] if len(x) > 0 else "NA" for x in self.byte_rep[-1]])
        if not is_labelized:
            # self.byte_rep[-1].pop(-1)
            self.byte_rep[-1].append(["T.label", label])
            self.label = label
            print(self.byte_rep, "===")
        else:
    ...
    def withLabel(self, label):
        ...
        if not is_labelized:
            ...
            # self.byte_rep[-1][-1].extend(["T.label", *label.get()])
            self.label = label
        else:
            el = [x[1] if "T.label" == x[0] else False for x in self.byte_rep[-1]]
            label = [x for x in el if x is not False]
