...
//  where  no.properties:userName = 'userName_0'
),
level2 as
(
...
//and  no.properties:userName = 'userName_2'
)
select *, 'root' as tbl from main
union all
select *, 'level1' as tbl from level1
union all
select *, 'level2' as tbl from level2
union all
select *, 'level' || lvl as tbl from repeat3_rcte
union all
select *, 'level5' as tbl from level5
union all
select *, 'level6' as tbl from level6
)
