...
class Generator:
    ...
    def load_config(self):
        ...
        for rplc in self.config['replaces']:
            ...
            self.replaces_sorted.append(rplc[0])
    ...
    def do_replace(self, line, replaces, ts):
        regexp = '(' + '|'.join(replaces.keys()) + ')'
        actual_keys = re.findall(regexp, line)
        already_replaced = []
        vals = {}
        for key_to_replace in self.replaces_sorted:
            if key_to_replace not in actual_keys:
                continue
            replace_to = ''
            ...
            if key_to_replace in replaces:
                ...
                if callable(values):
                    args = list(replaces[key_to_replace][1])
                    args = list(map(lambda itm: ts if str(itm) == '%TS_NUM%' else itm, args))
                    args = list(map(lambda itm: vals if str(itm) == '%VALS%' else itm, args))

                    replace_to = values(*args)
                else:
                    replace_to = values[random.randint(0, len(values)-1)]
            else:
                replace_to = 'DEFINE_ME_IN_CONFIG'

            if '%WIND_LOGON%' in replaces and key_to_replace == '%TARGET_ACCOUNT%' and replace_to in self.cron:
                cronline = self.cron[replace_to]

                if cronline != '' and not pycron.is_now(cronline, datetime.fromtimestamp(ts)):
                    return None

            vals[key_to_replace] = replace_to
            ...
            while end < end_of_period:
                for sample in tmpl['samples']:
                    ...
                    for i in range(0, msg_cnt):
                        ...
                        result = self.do_replace(sample, self.replaces, ts)
    ...
    def load_cron(self):
        config = self.load_yaml_from_file('wgtraffic_users.yaml')

        for k in config['ACTIVITY']:
            user = config['USERS'][k]['email'].split('@')[0]
            cron = config['ACTIVITY'][k][0][1][0]
            self.cron[user] = cron
